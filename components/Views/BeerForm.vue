...
export default {
  ...
  computed: {
    ...mapState(['beerInfo']),
    _id() {
      return this.beerInfo.beer._id
    },
    beerName: {
      set(beerName) {
        this.$store.commit('setBeer', { beerName })
      },
      get() {
        return this.beerInfo.beer.beerName
      }
    },
    brewery: {
      set(brewery) {
        this.$store.commit('setBeer', { brewery })
      },
      get() {
        return this.beerInfo.beer.brewery
      }
    },
    style: {
      set(style) {
        this.$store.commit('setBeer', { style })
      },
      get() {
        return this.beerInfo.beer.style
      }
    },
    degrees: {
      set(degrees) {
        this.$store.commit('setBeer', { degrees })
      },
      get() {
        return this.beerInfo.beer.degrees
      }
    },
    abv: {
      set(abv) {
        this.$store.commit('setBeer', { abv })
      },
      get() {
        return this.beerInfo.beer.abv
      }
    },
    tempBeer: {
      set(tempBeer) {
        this.$store.commit('setBeer', { tempBeer })
      },
      get() {
        return this.beerInfo.beer.tempBeer
      }
    },
    tempBrewery: {
      set(tempBrewery) {
        this.$store.commit('setBeer', { tempBrewery })
      },
      get() {
        return this.beerInfo.beer.tempBrewery
      }
    },
    rules() {
    ...
    breweries() {
      ...
      return this.me
        ? loadedBreweries
          .concat(this.preloadedBreweries)
          .sort((a, b) => a.localeCompare(b))
        : loadedBreweries
    },
    me() {
    ...
    editBeer() { return this.$store.state.editBeerDialog }
  },
  methods: {
    onSelect() {
      if (this.$refs.form.validate()) {
        this.$store.commit('toggle', 'loading')
        const testBrewery = this.editBeer ? this.brewery.name : this.brewery
        const checkForBrewery = this.$store.state.breweries.find(
          x => x.name === testBrewery
        )
        const brewery = checkForBrewery ? checkForBrewery._id : null

        if (this.editBeer) {
          this.$axios
            .patch(`/beers/${this._id}`, {
          ...
          const beer = this.$store.state.beers
            .filter(x => x.brewery._id === brewery)
          ...
          if (beer) {
            ...
            this.$axios
              ...
              .catch(() => {
                this.$store.commit('toggle', 'loading')
                this.$toast.error('Error adding beer, please try again', {
                  duration: 4000
                })
              })
          }
    ...
    onCancel() {
      ...
      this.$store.commit('truthy', { item: 'editBeerDialog', bool: false })
